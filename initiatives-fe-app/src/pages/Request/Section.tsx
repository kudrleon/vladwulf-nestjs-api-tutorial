...
export const Section = ({ description, questions, requestId, handleNext, isLastStep, activeStep, handleBack, updateProgressValue, isFinishDisabled }: props) => {
    ...
    return (
        <Fragment>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", pt: 2 }}>
            {[...(questions || [])].sort((({order}, {order: order2}) => order - order2)).map((question: question, index: number) => (
                <React.Fragment>
                    <QuestionAnswer
                        key={`${question.id}`}
          ...
          <Box sx={{ display: "flex", flexDirection: "row", pt: 2 }}>
            ...
            <Button 
                onClick={handleNext}
                disabled={isLastStep && isFinishDisabled}
            >
